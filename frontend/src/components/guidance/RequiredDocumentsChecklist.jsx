...
export default function RequiredDocumentsChecklist({ requiredDocuments = [] }) {
    ...
    <div className="rounded-2xl bg-slate-900 border border-slate-800 p-6 space-y-4 shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <h3 className="text-sm font-bold text-white uppercase tracking-wider text-indigo-400 flex items-center gap-2">
          <FileCheck2 size={18} /> Required Supporting Documents Checklist
        </h3>
        <span className="text-xs text-slate-400 font-medium">
          Prepare Before Next Step
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {requiredDocuments.map((doc, idx) => (
          ...
          >
            ...
            {doc.whyUseful && (
              <p className="text-[10px] text-indigo-300/90 pt-1 border-t border-slate-900">
                💡 <strong className="text-indigo-200">Why Useful:</strong> {doc.whyUseful}
              </p>
            )}
          </div>
        ))}
