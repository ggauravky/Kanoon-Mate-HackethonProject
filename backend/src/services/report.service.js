 ...
 * @returns {Promise<{ filePath: string, fileSize: number, reportName: string }>}
 */
export const buildLegalReportPDF = async (document, user) => {
  ...
  const fileName = `report-${document._id}-${Date.now()}.pdf`;
  const filePath = path.join('uploads/reports', fileName).replace(/\\/g, '/');
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      ...
      .text(
        analysis.disclaimer ||
          'This report is generated by LawAssist AI for informational purposes only. It is not legal advice and does not replace consultation with a qualified legal professional.',
        50,
        y + 20,
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc
        ...
        .fillColor('#94A3B8')
        .text(
          `Page ${i + 1} of ${range.count}  •  LawAssist AI Official Report`,
          40,
          doc.page.height - 30,
    ...
    writeStream.on('finish', async () => {
      try {
        ...
        resolve({
          filePath,
          fileSize: stats.size,
          reportName: `Legal_Report_${sanitizedTitle}.pdf`,
        });
      } catch (err) {
        reject(err);
      }
